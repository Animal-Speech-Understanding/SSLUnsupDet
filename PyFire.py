...
class Trainer(object):
	...
	def fit(self, train_loader, val_loader, epochs):
		...
		for epoch in range(start_epoch, epochs):
			...
			try:
				its = np.round(train_loader_len / endtime, decimals=2)
			except ZeroDivisionError:
				its = np.round(train_loader_len / (endtime+1e-100), decimals=2)
	...
	def train_step(self, dataloader, running_loss, running_metric=None):
		try:
			...
			total_len = len(list(dataloader))
            
		for i, data in tqdm.tqdm(enumerate(dataloader), total=total_len):
		...
		running_loss = [r_i / total_len for r_i in running_loss]
		try:
			running_metric = [r_i / total_len for r_i in running_metric]
		except:
			running_metric = None
